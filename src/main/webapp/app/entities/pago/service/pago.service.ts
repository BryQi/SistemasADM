...
type RestOf<T extends IPago | NewPago> = Omit<T, 'fechaPago' | 'createdAt'> & {
...
export class PagoService {
  ...
  protected convertDateFromClient<T extends IPago | NewPago | PartialUpdatePago>(pago: T): RestOf<T> {
    return {
      ...pago,
      fechaPago: pago.fechaPago?.format(DATE_FORMAT) ?? null,
      createdAt: pago.createdAt?.toJSON() ?? null,
    };
  }
  ...
  protected convertDateFromServer(restPago: RestPago): IPago {
    return {
      ...
      createdAt: restPago.createdAt ? dayjs(restPago.createdAt) : undefined,
    };
  }
